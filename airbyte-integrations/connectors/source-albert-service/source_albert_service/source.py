...
class AlbertServiceStream(HttpStream, ABC):
    """
    ...
    `class AlbertServiceStream(HttpStream, ABC)` which is the current class
    `class Customers(AlbertServiceStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(AlbertServiceStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalAlbertServiceStream((AlbertServiceStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
...
class Customers(AlbertServiceStream):
    """
...
class IncrementalAlbertServiceStream(AlbertServiceStream, ABC):
    """
...
class Employees(IncrementalAlbertServiceStream):
    """
    TODO: Change class name to match the table/data source this stream corresponds to.
    """
